# Eidoncore Platform — Documentation

Welcome to the Eidoncore platform documentation. This guide covers everything you need to manage your agency, serve your clients, and grow your business.

---

## 📚 Table of Contents

### Getting Started
- [**Getting Started**](./getting-started.md) — Create your workspace, first login, dashboard overview
- [**Dashboard**](./dashboard.md) — Executive, team member, and client dashboards with alerts and KPIs

### Core Features
- [**Projects**](./projects.md) — Create and manage projects, budgets, milestones, and team members
- [**Tasks**](./tasks.md) — Kanban boards, assignments, subtasks, checklists, time tracking, and comments
- [**Clients**](./clients.md) — CRM: manage client organizations, contacts, follow-ups, and health scoring
- [**Invoicing**](./invoicing.md) — Create invoices, record payments, recurring billing, credit notes, and finance analytics
- [**Services**](./services.md) — Build your service catalog, pricing models, cart & checkout, hours/credits tracking

### Team & Workflow
- [**Team Management**](./team.md) — Invite members, assign roles, custom permissions, workload dashboard
- [**Automations**](./automations.md) — Automate repetitive tasks with triggers, conditions, and actions
- [**Reports & Dashboards**](./reports.md) — Revenue KPIs, project health, task analytics, time tracking reports
- [**Notifications**](./notifications.md) — Bell notifications, email digests, priority levels, Slack integration

### Configuration
- [**Settings & Customization**](./settings.md) — Account, agency branding, email, domains, integrations, plans & billing
- [**Client Portal**](./client-portal.md) — What your clients see, portal roles, catalog purchases
- [**File Management**](./files.md) — Upload, organize, and manage files across your workspace
- [**Intake Forms**](./intake-forms.md) — Build custom forms, collect client requirements after purchase
- [**Security**](./security.md) — Two-factor authentication, session management, encryption, audit logs

---

## Quick Links

| I want to… | Go to |
|-------------|-------|
| Set up my workspace for the first time | [Getting Started](./getting-started.md) |
| Customize my branding & colors | [Settings](./settings.md#agency-branding) |
| Create my first project | [Projects](./projects.md#creating-a-project) |
| Invite my team | [Team Management](./team.md#inviting-team-members) |
| Add a client and give them portal access | [Clients](./clients.md#portal-access) |
| Set up automated workflows | [Automations](./automations.md) |
| Understand what my clients see | [Client Portal](./client-portal.md) |
| Choose the right plan for my agency | [Settings — Plans & Billing](./settings.md#plans--billing) |
